...
class SubtitleServer:
    ...
    async def register_audio(self, token: str, audio_path: str) -> bool:
        """登记瞬时 TTS 音频并广播给当前 overlay 客户端。"""
        token = str(token or "").strip()
        path = str(audio_path or "").strip()
        if not self._runner or not token or not path:
            return False
        try:
    ...
    async def _handle_audio(self, request: web.Request) -> web.StreamResponse:
        ...
        if not audio_file.is_file():
            self._audio_files.pop(token, None)
            raise web.HTTPNotFound()
        return web.FileResponse(audio_file, headers={"Cache-Control": "no-store"})
...
<body>
  ...
  <audio id="tts-audio" hidden preload="auto" aria-hidden="true"></audio>
  <script>
