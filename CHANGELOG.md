...
- SC 默认进入保证回应队列：跳过普通冷却、最少事件数、读空气和限流，按事件 ID 覆盖待处理、处理中和已回应三个阶段去重；发送前兜底补齐实名鸣谢，并要求继续回应 SC 正文。若 native 与直接 LLM 均未产出，最终发送不依赖模型的纯文字点名鸣谢与正文接收确认，避免静默吞掉 SC。
- 询问“今天/最近看了什么”时直接读取 BiliBot 活动记忆并注入 native 回复；禁止重复调用 `recall_today`，同时限制 Agent 步数并对停留在“让我查一下”的无效回复执行回退。
