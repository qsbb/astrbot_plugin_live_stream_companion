...
> **拓展页「直播面板 → 配置 → 演出连接（VTS / L2DStudio）」**已把 VTS 相关入口收在一处：
> - **VTS 地址**：下拉选择探测到的实例（`IP:端口 · VTS 版本`），旁边有「扫描局域网」「测试连接」；选中后自动带出端口。
> - **认证 VTS**：直接点按钮即可（等价于聊天里的 `/vts_auth`），VTS 弹窗允许后即时显示连接状态。
> - **自动连接 / 自动发现 / 提及提示**、**Live2D 表情按键映射**（可从 VTS 读取热键一键生成模板）与 **L2DStudio 路径**都在同一组。
> AstrBot 与 VTS 不在同一台机器时，只需填局域网 IP，不用手抄端口。
5. 检查连接：
...
> 拓展页里 OBS 的入口也已对齐：总览「OBS 开播控制」卡片右上角有 **「配置 OBS」**（一键跳到配置页的 OBS 分组并高亮），
> 未启用时会显示可点击的「去启用 OBS 开播控制」；`obs_live_scene_name` 支持**从 OBS 读取场景列表直接下拉**，
> 动作按钮按「启动 / 控制 / 直播」三组分开，开始与停止直播单独成组并保留二次确认。
